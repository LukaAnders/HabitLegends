...
export function Kingdom() {
  const { user } = useAuth(); const { tasks, loading } = useTasks(); const { isCompleted } = useTaskCompletions(); const { claimed: dailyClaimed } = useDailyReward(); const [busy, setBusy] = useState(false); const [dailyBusy, setDailyBusy] = useState(false); const [error, setError] = useState<string | null>(null); const [reward, setReward] = useState<CompletionReward | null>(null); const [levelUp, setLevelUp] = useState<CompletionReward | null>(null)
  ...
  async function openDailyChest(){if(!user||dailyBusy)return;setDailyBusy(true);setError(null);try{await claimDailyReward(user.uid,completed,tasks.length)}catch(cause){setError(getFriendlyFirebaseError(cause))}finally{setDailyBusy(false)}}
  return <main className="kingdom-page"><HeroStage />
    ...
    <section className="kingdom-section grid gap-5 xl:grid-cols-[1.55fr_.75fr]"><JourneyPath /><DailyChest completed={completed} total={tasks.length} claimed={dailyClaimed} busy={dailyBusy} onClaim={openDailyChest} /></section><section className="kingdom-section"><MerchantPreview /></section>
